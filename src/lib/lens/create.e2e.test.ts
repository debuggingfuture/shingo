  ...
  it("create mocks posts with comments", async () => {
    ...
    await Promise.all(
      items.map(async (metadata, i) => {
        ...
        const item = _.first(publications.items);
        console.log("item", item);
        await Promise.all(
          metadata.comments.map(async (commentMetadata, i) => {
            return createComment(item.id, wallet, profileIds[i], {
              ...commentMetadata,
            });
          })
        );
